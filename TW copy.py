...
restaurants = [
    ...
    },
    {
        ...
        'address': '104053台湾台北市中山區樂群三路200號5樓, Taiwan'
    },
    {
        'name': '饃饃噠心動肉夾饃',
    ...
    {
        'name': 'Bounceback 美式漢堡',
        'address': '220台湾新北市板橋區裕民街60號, Taiwan'
    },
    {
        'name': '士林夜市銅鑼燒',
        'address': '士林夜市, Taiwan'
    ...
    if places['status'] == 'OK':
        try:
            if places['results'][0]['rating'] < 3.9:
                ...
                restaurant['lat'],restaurant['lng'] = locations[0],locations[1]
            ...
            # Construct the direct link to the restaurant using the place ID
                ...
                #print(restaurant)
                        ...
                        print(f"no results for {restaurant['name']}",search_results)############
...
for res in rest_res:
    ...
    if "link" in res:
        restaurant_data["link"]= res["link"]
    else:
        print('No id,no link')
        restaurant_data["link"]= "Empty Link"
...
#########################################################################
#route search
# Specify travel modes (exclude 'driving')

# Define your endpoint
endpoint = 'https://maps.googleapis.com/maps/api/directions/json'

# Define start and end points (latitude, longitude)
start_point = '25.0877916,121.5244812'  # Example
end_point = '25.0932719,121.526611'  # Example

# Define transit waypoints (if any)
transit_waypoints = [
    '25.0880648,121.5249608',  # Replace with the latitude and longitude of your transit stop
...
if response.status_code == 200:
    ...
    if routes:
        ...
        print("No routes found for walking.")
...
# Bus, subway modes
travel_modes1 = ['transit']

# Construct the API request URL
params = {
    'origin': start_point,
    'destination': end_point,
    'mode': '|'.join(travel_modes1),  # Combine travel modes with a pipe (|) separator
    'waypoints': '|'.join(transit_waypoints),  # Combine transit waypoints with a pipe (|) separator
    'key': google_api_key,
    'avoid': 'tolls',  # Optionally, you can specify avoidance preferences
}
...
if response.status_code == 200:
    ...
    for route in routes:
        ...
        print("No routes found for transit.")
    # Process the directions and steps in the response
else:
    print(f"Request failed with status code {response.status_code}")


# Replace with the latitude and longitude of your restaurant locations
waypoints = ['25.0392193,121.4986043', '25.0313085,121.5600484', '25.0134436,121.5353398']
...
with open('ilu6.html', 'w', encoding='utf-8') as f:
